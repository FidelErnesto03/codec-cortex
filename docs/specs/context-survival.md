...
La eficiencia se mide por cuántas decisiones, restricciones, foco, objetivo, estado y próximo paso sobreviven por token usado. El conjunto exacto de campos críticos se estabilizará con benchmarks posteriores. Esta métrica es conceptual — no pretende ser una medición automatizada hasta que exista parser.
