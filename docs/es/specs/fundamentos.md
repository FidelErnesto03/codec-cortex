...
> **NOTA DE ESTADO:** Este documento es especificacion o diseno. A v0.3.6 el CLI y codec determinista (parse, encode, decode, verify, HCORTEX render, canonicalize, convert, roundtrip-bidir, inspect), la capa de seguridad E2 (`cortex doctor --scan-secrets`, `cortex audit`, `cortex --mode`, `cortex verify --signature`) y el protocolo de documentacion E3 (`docs/cortex/api/*.cortex`, `cortex docstring`, `cortex benchmark`) estan implementados en cli/. El runtime y el servidor MCP siguen siendo planificados o futuros.
