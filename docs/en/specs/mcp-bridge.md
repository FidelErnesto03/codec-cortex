...
> **STATUS NOTE:** This document is specification or design. As of v0.3.6 the codec, CLI (including E2 security and E3 documentation protocol), and HCORTEX render are implemented in cli/. The runtime lifecycle and the MCP server itself remain planned or future; the handler map below is the design contract that the future MCP server will expose.
