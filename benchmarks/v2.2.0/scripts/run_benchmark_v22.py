...
def _get_hcortex_v2(case_id: str, profile: str) -> str:
    ...
    cortex_path = SRC / f"{case_id}.cortex"
    tmp_out = f"/tmp/v22_bench_convert_{case_id}_{profile}.md"
    try:
        subprocess.run(
...
def select_by_method(method, case, source_text, budget, scenario_id, task):
    if method.config.get("use_cli"):
        ...
        if method.config.get("selector") == "cortex_cli_v1":
            ...
            tmp_canon = f"/tmp/v22_bench_canon_{case['case_id']}.cortex"
            try:
                subprocess.run(
                ...
                if os.path.exists(tmp_canon):
                    tmp_out = f"/tmp/v22_bench_canon_convert_{case['case_id']}_{profile}.md"
                    subprocess.run(
                        CORTEX_CLI + ["convert", "--from", "cortex", "--to", "hcortex",
...
# ---------------------------------------------------------------------------
# NEW v2.2.0: Learning Engine metrics
# ---------------------------------------------------------------------------

_LEARN_CACHE: Dict[str, Dict] = {}
_LEARN_WORKSPACE_DIR = Path("/home/z/my-project/download/benchmark-cortex-v22/learning_workspaces")


def _setup_learn_workspace(case_id: str) -> Path:
    """Setup a learning workspace for a case (cached)."""
    ws_path = _LEARN_WORKSPACE_DIR / case_id
    cortex_dir = ws_path / ".cortex"
    if cortex_dir.exists():
        return ws_path
    cortex_dir.mkdir(parents=True, exist_ok=True)
    (cortex_dir / "index").mkdir(exist_ok=True)
    (cortex_dir / "cache").mkdir(exist_ok=True)
    # Copy the case's .cortex as brain.cortex
    src_cortex = SRC / f"{case_id}.cortex"
    (cortex_dir / "brain.cortex").write_text(src_cortex.read_text())
    # Copy default policies and manifest from template
    import shutil
    template_ws = Path("/tmp/v22_test_workspace/.cortex")
    if (template_ws / "MANIFEST.cortex").exists():
        shutil.copy2(template_ws / "MANIFEST.cortex", cortex_dir / "MANIFEST.cortex")
    if (template_ws / "learn-policies.cortex").exists():
        shutil.copy2(template_ws / "learn-policies.cortex", cortex_dir / "learn-policies.cortex")
    return ws_path


def _run_learn_scan(case_id: str) -> Dict:
    """Run cortex learn scan and return parsed JSON (cached)."""
    if case_id in _LEARN_CACHE:
        return _LEARN_CACHE[case_id]
    ws_path = _setup_learn_workspace(case_id)
    try:
        r = subprocess.run(
            CORTEX_CLI + ["learn", "scan", "--workspace", str(ws_path), "--json"],
            capture_output=True, text=True, timeout=30,
        )
        if r.returncode == 0 and r.stdout.strip():
            # Try to parse as JSON; if not JSON, extract counts from text
            try:
                data = json.loads(r.stdout)
            except json.JSONDecodeError:
                data = {"raw_output": r.stdout, "entries": []}
        else:
            data = {"entries": [], "error": r.stderr[:200] if r.stderr else "unknown"}
    except Exception as e:
        data = {"entries": [], "error": str(e)[:200]}
    _LEARN_CACHE[case_id] = data
    return data


def _run_learn_candidates(case_id: str) -> List[Dict]:
    """Run cortex learn candidates and return list (cached)."""
    ws_path = _setup_learn_workspace(case_id)
    cache_key = f"{case_id}_candidates"
    if cache_key in _LEARN_CACHE:
        return _LEARN_CACHE[cache_key]
    try:
        r = subprocess.run(
            CORTEX_CLI + ["learn", "candidates", "--workspace", str(ws_path), "--json"],
            capture_output=True, text=True, timeout=30,
        )
        if r.returncode == 0 and r.stdout.strip():
            try:
        ...
        candidates = []
    _LEARN_CACHE[cache_key] = candidates
    return candidates


def compute_learn_candidate_count(case_id: str) -> int:
    """Number of learning candidates detected by cortex learn scan."""
    data = _run_learn_scan(case_id)
    entries = data.get("entries", [])
    return len(entries) if isinstance(entries, list) else 0


def compute_learn_promotion_score(case_id: str) -> float:
    """Average promotion score of detected candidates."""
    data = _run_learn_scan(case_id)
    entries = data.get("entries", [])
    if not entries or not isinstance(entries, list):
        return 0.0
    scores = [e.get("promotion_score", 0) for e in entries if isinstance(e, dict)]
    return sum(scores) / len(scores) if scores else 0.0


def compute_learn_elevation_candidates(case_id: str) -> int:
    """Number of candidates with suggested_action='consider_elevation'."""
    data = _run_learn_scan(case_id)
    entries = data.get("entries", [])
    if not entries or not isinstance(entries, list):
        return 0
    return sum(1 for e in entries if isinstance(e, dict) and e.get("suggested_action") == "consider_elevation")


def compute_learn_hotness_avg(case_id: str) -> float:
    """Average hotness score of detected candidates."""
    data = _run_learn_scan(case_id)
    entries = data.get("entries", [])
    if not entries or not isinstance(entries, list):
        return 0.0
    scores = [e.get("hotness_score", 0) for e in entries if isinstance(e, dict)]
    return sum(scores) / len(scores) if scores else 0.0


# ---------------------------------------------------------------------------
# NEW v2.2.0: E2 Security metrics
# ---------------------------------------------------------------------------

_SECURITY_CACHE: Dict[str, Dict] = {}


def compute_secret_scan(case_id: str) -> Dict:
    """Run cortex doctor --scan-secrets and return findings (cached)."""
    if case_id in _SECURITY_CACHE:
        return _SECURITY_CACHE[case_id]
    cortex_path = SRC / f"{case_id}.cortex"
    try:
        r = subprocess.run(
            CORTEX_CLI + ["doctor", "--scan-secrets", str(cortex_path), "--json"],
            capture_output=True, text=True, timeout=15,
        )
        if r.returncode == 0 and r.stdout.strip():
            try:
                ...
                data = {"secrets_found": 0, "raw": r.stdout[:200]}
        else:
            # doctor --scan-secrets may not support --json; parse text
            data = {"secrets_found": 0, "stderr": r.stderr[:200] if r.stderr else ""}
            if "secret" in r.stdout.lower() if r.stdout else "":
                data["secrets_found"] = r.stdout.lower().count("secret")
    except Exception as e:
        data = {"secrets_found": 0, "error": str(e)[:200]}
    _SECURITY_CACHE[case_id] = data
    return data


def compute_secret_count(case_id: str) -> int:
    """Number of secrets detected by cortex doctor --scan-secrets."""
    data = compute_secret_scan(case_id)
    return data.get("secrets_found", 0)


# ---------------------------------------------------------------------------
# Ejecucion del benchmark v2.2.0
# ---------------------------------------------------------------------------
...
def run_benchmark():
    ...
    print("\nPre-computing v2.2.0 Learning Engine scan per case...")
    for case in cases:
        lc = compute_learn_candidate_count(case["case_id"])
        lp = compute_learn_promotion_score(case["case_id"])
        le = compute_learn_elevation_candidates(case["case_id"])
        lh = compute_learn_hotness_avg(case["case_id"])
        sc = compute_secret_count(case["case_id"])
        print(f"  {case['case_id']}: candidates={lc} prom_score={lp:.2f} elevations={le} hotness={lh:.2f} secrets={sc}")
    ...
    for method in METHODS:
        ...
        for scenario_id, scenario_fn, budget in SCENARIOS:
            for task in all_tasks:
                ...
                # NEW v2.2.0: Learning Engine metrics (computed for all CODEC methods)
                is_cortex = method.family in ("cortex_v1", "cortex_v2")
                if is_cortex and scenario_id == "full":
                    learn_candidates = compute_learn_candidate_count(case["case_id"])
                    learn_prom_score = compute_learn_promotion_score(case["case_id"])
                    learn_elevations = compute_learn_elevation_candidates(case["case_id"])
                    learn_hotness = compute_learn_hotness_avg(case["case_id"])
                    secret_count = compute_secret_count(case["case_id"])
                else:
                    learn_candidates = 0
                    learn_prom_score = 0.0
                    learn_elevations = 0
                    learn_hotness = 0.0
                    secret_count = 0

                # Weighted score (extended for v2.2.0 with learning metrics)
                weighted = (
                    eas * 1.0 + etc * 1.5 + f1 * 1.5 + da * 2.0 +
                    p0_surv * 2.0 + p1_surv * 1.0 + str_rate * 1.0 +
                    view_cov * 1.5 +  # v2.0: VIEW coverage
                    reversibility * 1.0 +  # v2.0: reversibility
                    # v2.2.0: learning engine contributions (small weights, diagnostic)
                    min(learn_candidates * 0.1, 1.0) +  # cap at 1.0
                    min(learn_prom_score * 0.1, 1.0) +  # cap at 1.0
                    - secret_count * 2.0  # penalize secrets found
                ...
                scored_rows.append({
                    ...
                    "STR": str_rate,
                    "BVR": bvr,
                    "VIEW_coverage": round(view_cov, 4),  # v2.0
                    "reversibility": reversibility,  # v2.0
                    "bidir_equivalence": bidir_equiv,  # v2.0
                    "loss_count": loss_count,  # v2.0
                    "learn_candidates": learn_candidates,  # v2.2 NEW
                    "learn_promotion_score": round(learn_prom_score, 4),  # v2.2 NEW
                    "learn_elevations": learn_elevations,  # v2.2 NEW
                    "learn_hotness_avg": round(learn_hotness, 4),  # v2.2 NEW
                    "secret_count": secret_count,  # v2.2 NEW (E2 security)
    ...
    for row in scored_rows:
        ...
        for k in ["EAS", "ETC", "F1", "DA", "P0_survival", "P1_survival", "BCFNR", "UCFPR", "CFCR", "STR", "BVR",
                  ...
                  "learn_candidates", "learn_promotion_score", "learn_elevations", "learn_hotness_avg", "secret_count",
    ...
    metric_registry = {
        ...
        "metrics": [
            ...
            # NEW v2.2.0 metrics (Learning Engine + E2 Security)
            {"id": "learn_candidates", "name": "Learning Candidates Detected", "type": "count", "range": "0..inf", "higher_better": True, "status": "canonical", "introduced_in": "2.2.0", "engine": "CLE v0.1.0"},
            {"id": "learn_promotion_score", "name": "Learning Promotion Score (avg)", "type": "ratio", "range": "0..inf", "higher_better": True, "status": "canonical", "introduced_in": "2.2.0", "engine": "CLE v0.1.0"},
            {"id": "learn_elevations", "name": "Learning Elevation Candidates", "type": "count", "range": "0..inf", "higher_better": True, "status": "canonical", "introduced_in": "2.2.0", "engine": "CLE v0.1.0"},
            {"id": "learn_hotness_avg", "name": "Learning Hotness Score (avg)", "type": "ratio", "range": "0..inf", "higher_better": True, "status": "canonical", "introduced_in": "2.2.0", "engine": "CLE v0.1.0"},
            {"id": "secret_count", "name": "Secrets Detected (E2)", "type": "count", "range": "0..inf", "higher_better": False, "status": "canonical", "introduced_in": "2.2.0", "engine": "E2 Security v0.3.4"},
        ],
    }
    ...
    manifest = {
        ...
        "new_in_v2.2": {
            "learning_engine": "CODEC-CORTEX Learning Engine (CLE) v0.1.0",
            "learning_metrics": ["learn_candidates", "learn_promotion_score", "learn_elevations", "learn_hotness_avg"],
            "security_metrics": ["secret_count"],
            "security_engine": "E2 Security v0.3.4 (secret scanner, mutation gates, audit log)",
            "cli_commands_new": ["learn init", "learn doctor", "learn scan", "learn candidates", "learn explain", "learn elevate", "audit", "doctor --scan-secrets", "verify --signature"],
            "skill_version": "v1.3.0 (SKILL reescrito en HCORTEX con 35 VIEW directives)",
            "agent_md_updates": "AGENT.md con KNW entries para todos los comandos CLI v0.3.2+",
        },
        "totals": {
            "methods": len(METHODS),
