...
- Métodos nuevos: cortex_v2_priority_pack, cortex_v2_canonical
...
METHODS: List[MethodSpec] = [
    ...
    MethodSpec("cortex_v2_priority_pack", "CORTEX v2 PP (CLI v2.4 v2-convert)", "cortex_v2",
    ...
    # CODEC-CORTEX v2 canonical (NEW: v2-canonicalize primero, luego render)
    MethodSpec("cortex_v2_canonical", "CORTEX v2 Canonical (normalize + render)", "cortex_v2",
               "v2.0.0: v2-canonicalize seguido de v2-convert. Produces normalized HCORTEX.",
               {"source": "cortex", "selector": "cortex_v2_canonical", "use_cli": True}, is_v2=True),
...
def _get_hcortex_v2(case_id: str, profile: str) -> str:
    """Render via CLI v2.4 v2-convert --from cortex --to hcortex.

    v2-convert produces HCORTEX with VIEW directives. If the .cortex has no
    VIEW directives (legacy corpus), the output is minimal (just metadata).
    In that case, we fall back to v1 render --profile for substantive content.

    This is a key finding of benchmark v2.0.0: v2-convert requires VIEW
    directives in the source .cortex to produce meaningful HCORTEX output.
    """
    key = (case_id, profile)
    if key in _HC_CACHE_V2:
        return _HC_CACHE_V2[key]
    cortex_path = SRC / f"{case_id}.cortex"
    tmp_out = f"/tmp/v2_bench_render_v2_{case_id}_{profile}.md"
    try:
        subprocess.run(
            CORTEX_CLI + ["v2-convert", "--from", "cortex", "--to", "hcortex",
        ...
        if os.path.exists(tmp_out):
            ...
            if len(rendered) < 500:
                # Fallback: use v1 render --profile (legacy)
...
def _get_v2_view_info(case_id: str) -> Dict:
    ...
    try:
        r = subprocess.run(
            CORTEX_CLI + ["v2-verify-view", str(cortex_path), "--format", "json"],
            capture_output=True, text=True, timeout=15,
        )
...
def select_by_method(method, case, source_text, budget, scenario_id, task):
    if method.config.get("use_cli"):
        ...
        elif method.config.get("selector") == "cortex_v2_canonical":
            # First canonicalize, then convert
            cortex_path = SRC / f"{case['case_id']}.cortex"
            tmp_canon = f"/tmp/v2_bench_canon_{case['case_id']}.cortex"
            try:
                subprocess.run(
                    CORTEX_CLI + ["v2-canonicalize", str(cortex_path), "--out", tmp_canon],
                    capture_output=True, text=True, timeout=15,
                )
                if os.path.exists(tmp_canon):
                    tmp_out = f"/tmp/v2_bench_canon_render_{case['case_id']}_{profile}.md"
                    subprocess.run(
                        CORTEX_CLI + ["v2-convert", "--from", "cortex", "--to", "hcortex",
...
def compute_bidir_equivalence(case_id):
    ...
    try:
        r = subprocess.run(
            CORTEX_CLI + ["v2-roundtrip-bidir", str(cortex_path)],
...
def compute_loss_explained_count(case_id):
    ...
    try:
        r = subprocess.run(
            CORTEX_CLI + ["v2-explain-loss", str(cortex_path), "--format", "json"],
            capture_output=True, text=True, timeout=15,
        )
        if r.returncode == 0 and r.stdout.strip():
...
def run_benchmark():
    ...
    v2_methods = ["cortex_v2_priority_pack", "cortex_v2_canonical"]
    ...
    manifest = {
        ...
        "new_in_v2": {
            "methods": ["cortex_v2_priority_pack", "cortex_v2_canonical"],
            "metrics": ["VIEW_coverage", "reversibility", "bidir_equivalence", "loss_count"],
            "cli_commands_used": ["v2-convert", "v2-verify-view", "v2-roundtrip-bidir", "v2-canonicalize", "v2-explain-loss"],
        },
        "totals": {
            "methods": len(METHODS),
